...
def load_dataset(args, corpus_type, shuffle):
    ...
    # pts = sorted([x for x in os.listdir(args.bert_data_path) if corpus_type in x])
    pts = sorted([str(x) for x in Path(args.bert_data_path).iterdir() if corpus_type in str(x)])
    if pts:
        if shuffle:
            random.shuffle(pts)
